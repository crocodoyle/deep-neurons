...
dataset_dir = '/data1/users/adoyle/deepneuronsdataset/'
...
class generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Linear(1024, 128 * (self.input_width // 8) * (self.input_height // 8)),
            nn.BatchNorm1d(128 * (self.input_width // 8) * (self.input_height // 8)),
            nn.ReLU(),
        )
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(128, 64, self.kernel_size, self.conv_stride, self.padding),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.ConvTranspose2d(64, 32, self.kernel_size, self.conv_stride, self.padding),
            nn.ReLU(),
            nn.ConvTranspose2d(32, self.output_dim, self.kernel_size, self.conv_stride, self.padding),
            nn.Sigmoid(),
        )
    ...
    def forward(self, input):
        x = self.fc(input)
        x = x.view(x.size(0), 128, (self.input_width // 8), (self.input_height // 8))
        x = self.deconv(x)
...
class discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(self.input_dim, 64, self.kernel_size, self.conv_stride, self.padding),
            nn.LeakyReLU(0.2),
            nn.Conv2d(64, 128, self.kernel_size, self.conv_stride, self.padding),
            nn.LeakyReLU(0.2),
            nn.Conv2d(64, 128, self.kernel_size, self.conv_stride, self.padding),
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2),
        ...
        self.flat_features = self.get_flat_features((self.input_dim, self.input_width, self.input_height), self.features)
...
class WGAN_GP(object):
    def __init__(self, args):
        ...
        self.data_loader = utils.load_deep_neurons(dataset_dir, transform=transforms.Compose([transforms.ToTensor()]), batch_size=self.batch_size)
        self.z_dim = 62
