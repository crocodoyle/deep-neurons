...
def load_data():
    f = h5py.File('deep-neurons.hdf5', 'r')
...
#    model = models.mlp(3)
    ...
    model.compile(loss='categorical_crossentropy',
                  optimizer=sgd,
                  metrics=["accuracy"])
#    x_train = np.reshape(np.asarray(images)[train_index,...], (len(np.asarray(images)[train_index,...]), 128*128))
    x_train = np.reshape(np.asarray(images)[train_index],(593,128,128,1))
